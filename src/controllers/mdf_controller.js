...
exports.get_EpsScr_bycell = async function(req, res) {
    ...
    var min_scr = d3.min(data_json.map(function(d) {return parseFloat(d.total_score);}));
    debug("min_scr: " + min_scr)

    var max_scr = d3.max(data_json.map(function(d) {return parseFloat(d.total_score);}));
    debug("max_scr: " + max_scr)
    ...
    var scr_dimension = cross_species.dimension(function(d) { return parseFloat(d.total_score); });
